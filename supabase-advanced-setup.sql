...
-- This demonstrates TRUE PostgreSQL power vs Firebase limitations
...
CREATE OR REPLACE FUNCTION get_user_analytics(user_id TEXT)
RETURNS JSON AS $$
DECLARE
    result JSON;
BEGIN
    ...
    FROM (
        ...
        FROM (
            SELECT 
                ...
                COUNT(*) OVER (PARTITION BY p.name) as project_count
            FROM tasks t
...
CREATE OR REPLACE FUNCTION get_productivity_insights(user_id TEXT, days_back INT DEFAULT 30)
RETURNS JSON AS $$
DECLARE
    result JSON;
BEGIN
    SELECT json_build_object(
        ...
        'ai_task_success_rate', ai_success_rate
    ) INTO result
    FROM (
        SELECT 
            json_agg(
            ...
            mode() WITHIN GROUP (ORDER BY EXTRACT(dow FROM updated_at)) as peak_day,
            mode() WITHIN GROUP (ORDER BY EXTRACT(hour FROM updated_at)) as peak_hour,
            
            -- Calculate completion velocity (tasks per day trend)
            CASE 
                WHEN COUNT(*) > 7 THEN
                    ROUND(
        ...
        GROUP BY (
            SELECT json_agg(
                json_build_object(
                    ...
                    'ai_enhanced_completed', COUNT(*) FILTER (WHERE ai_enhanced = true AND archived = true)
                )
            )
            FROM tasks t2
            WHERE t2.user_id = user_id 
                AND t2.archived = true
                AND t2.updated_at >= NOW() - (days_back || ' days')::INTERVAL
...
BEGIN
    ...
    -- For now, we'll just ensure the updated_at is current
...
CREATE OR REPLACE FUNCTION search_tasks(user_id TEXT, search_query TEXT)
RETURNS TABLE(
    id INTEGER,
    task TEXT,
    project_name TEXT,
